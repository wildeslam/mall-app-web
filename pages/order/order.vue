				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData('add')">
					...
					<view v-for="(item,index) in orderList" :key="index" class="order-item">
						...
						<view class="action-box b-t" v-if="item.status == 0">
							<button class="action-btn" @click="cancelOrder(item.id)">CANCLE</button>
							<button class="action-btn recom" @click="payOrder(item.id)">PAY NOW</button>
						</view>
						<view class="action-box b-t" v-if="item.status == 2">
							<button class="action-btn" >SHIPMENT</button>
							<button class="action-btn recom" @click="receiveOrder(item.id)">RECEIVE</button>
						</view>
						<view class="action-box b-t" v-if="item.status == 3">
							<button class="action-btn recom" >Make Review</button>
						</view>
					</view>
	...
	export default {
		...
		filters: {
			formatStatus(status) {
				...
				switch (+status) {
					case 0:
						statusTip = 'Waiting For Payment';
						break;
					case 1:
						statusTip = 'Waiting For Shipment';
						break;
					case 2:
						statusTip = 'Waiting For Receipt';
						break;
					case 3:
						statusTip = 'Done';
						break;
